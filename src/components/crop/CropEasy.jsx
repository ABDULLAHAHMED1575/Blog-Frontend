  ...
  return (
    <div className='fixed z-[1000] inset-0 bg-black/50 flex justify-center p-5 overflow-auto'>
        <div className='bg-white h-fit w-full sm:max-w-[350px] p-5 rounded-lg'>
            <h2 className='font-semibold text-dark-hard mb-2'>
                Crop Image
            </h2>
            <div className='relative w-full aspect-square rounded-lg overflow-hidden'>
                <Cropper image={photo?.url} crop={crop} 
                zoom={zoom} aspect={1} onZoomChange={setZoom} 
                onCropChange={setCrop} onCropComplete={handleCropComplete}
                />
            </div>
            <div>
                <label htmlFor="zoomRange" className='block mt-2 mb-0.5 text-sm font-medium text-gray-900'>
            ...
            <div className='flex justify-between gap-2 flex-wrap'>
                ...
                onClick={()=> setOpenCrop(false)}
                 className='px-5 py-2.5 rounded-lg border text-red-500 border-red-500 disabled:opacity-70'>
                    cancel
                </button>
                <button
                disabled={isLoading}
                onClick={()=> handleCropImage()}
                 className='px-5 py-2.5 rounded-lg text-white bg-blue-500 disabled:opacity-70'>
                    Upload
                </button>
            </div>
        </div>
